...
rollerIds=O.bodies.append(ymport.stl('roller-Layer2.stl',wire=True))
firstLayerIds=O.bodies.append(ymport.stl('contour_layer1.stl',wire=True))

sp=pack.SpherePack()
sp.makeCloud((0,0,100),(1500,200,350),
...
def checkUnbalanced_2():
   if unbalancedForce()<.01 or O.time>(time2+1):
      ...
      global sumVolume
      for b in O.bodies:
         if isinstance(b.shape, Sphere) and b.state.pos[2]<=60 and b.state.pos[2]>=0 and b.state.pos[0]<=1000 and b.state.pos[0]>=0 and b.state.pos[1]<=200 and b.state.pos[1]>=0:
