...
bladeIds=O.bodies.append(ymport.stl('blade.stl',wire=True))
...
def startPlating():
   global time2
   time2=O.time
   O.engines=O.engines[:5]+[TranslationEngine(translationAxis=(1,0,0),velocity=5,ids=bladeIds)]+[PyRunner(command='stopPlating()',realPeriod=2)]

def stopPlating():
   if O.time>(time2+16):
      global time3
      time3=O.time
      O.engines=O.engines[:5]+[TranslationEngine(ids=bladeIds)]+[TranslationEngine(translationAxis=(0,0,-1),velocity=2,ids=baseIds)]+[PyRunner(command='unloadBase()',realPeriod=2)]

def unloadBase():
   if O.time>(time3+5):
      global time4
      time4=O.time
      O.engines=O.engines[:5]+[TranslationEngine(ids=baseIds)]+[TranslationEngine(translationAxis=(1,0,0),velocity=5,ids=bladeIds)]+[PyRunner(command='unloadPlate()',realPeriod=2)]

def unloadPlate():
   if O.time>(time4+4):
      global time5
      time5=O.time
      O.engines=O.engines[:5]+[TranslationEngine(ids=bladeIds)]+[PyRunner(command='checkUnbalanced_2()',realPeriod=2)]
